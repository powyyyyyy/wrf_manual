...
---
📋 Daftar Isi
Pendahuluan & Prerequisites
Konfigurasi Domain (Nesting)
Persiapan Data Input
Fase WPS-4.3 (Preprocessing)
Fase WRF-4.3 (Core Model Running)
Troubleshooting & Tips Efisiensi
...
1.2 Prerequisites
Komponen	Keterangan
Environment	Docker Container (`dtcenter/wps_wrf`) di atas WSL2 (Windows) / native Linux Ubuntu
...
```
1.4 Menjalankan Container dengan Volume Mounting
Agar data (namelist, hasil `geogrid`/`ungrib`/`metgrid`, dan output `wrfout`) tidak hilang saat container dihentikan atau dihapus, wajib melakukan bind mount folder lokal ke dalam container:
```bash
docker run -it \
  --name wrf_cempaka_2017 \
  -v /path/lokal/anda/WPS_WRF_WORKDIR:/home/wrf/workdir \
  -v /path/lokal/anda/WPS_GEOG:/home/wrf/WPS_GEOG \
  dtcenter/wps_wrf \
  /bin/bash
```
> 💡 **Catatan:** Ganti `/path/lokal/anda/...` sesuai direktori kerja Anda. Untuk masuk kembali ke container yang sudah pernah dibuat (bukan membuat baru), gunakan:
> ```bash
> docker start -ai wrf_cempaka_2017
> ```
---
...
```
> 📌 Pastikan path hasil ekstraksi ini nantinya dirujuk persis pada variabel `geog_data_path` di `&geogrid` (lihat bagian 4.1).
...
Semua langkah berikut dijalankan di dalam container, pada direktori `/WPS`.
```bash
cd /WPS
```
...
```bash
./link_grib.csh /path/ke/data/ERA5_PL_cempaka_20171126_29.grib
./link_grib.csh /path/ke/data/ERA5_SFC_cempaka_20171126_29.grib
```
c) Eksekusi Ungrib:
```bash
...
6.2 Manajemen Storage: Zero-Filling untuk WSL2/Docker Windows
Output `wrfout` dan `wrfrst` berukuran sangat besar. Di WSL2, menghapus file tidak otomatis mengecilkan ukuran virtual disk (`ext4.vhdx`) karena sifatnya sparse file. Solusinya adalah teknik zero-filling sebelum kompaksi:
Langkah 1 — Hapus file besar yang sudah tidak diperlukan (setelah visualisasi selesai):
```bash
rm -f wrfout_d0*_2017-11-2[6-9]* wrfrst_d0*
```
Langkah 2 — Isi ruang kosong bekas file tersebut dengan nol (zero-fill), lalu hapus:
```bash
dd if=/dev/zero of=sampah.txt bs=1M
# biarkan berjalan hingga muncul error "No space left on device"
rm -f sampah.txt
```
Langkah 3 — Kompaksi VHDX dari sisi Windows (PowerShell/CMD sebagai Administrator):
```powershell
wsl --shutdown
diskpart
```
Di dalam prompt `diskpart`:
```
select vdisk file="C:\Users\<UserName>\AppData\Local\Packages\<DistroPackage>\LocalState\ext4.vhdx"
attach vdisk readonly
compact vdisk
detach vdisk
exit
```
> 💽 Teknik ini secara efektif mengembalikan ruang harddisk fisik di Windows yang sebelumnya "terkunci" oleh file `wrfout`/`wrfrst` yang sudah dihapus namun belum ter-*reclaim* oleh virtual disk WSL2.
---
7. 📂 Struktur Direktori Proyek
```
WRF_CEMPAKA_2017/
├── WPS_GEOG/                     # Data statis geografis (di-mount ke container)
├── data_era5/
│   ├── ERA5_PL_cempaka_20171126_29.grib
│   └── ERA5_SFC_cempaka_20171126_29.grib
├── WPS_WRF_WORKDIR/               # Working directory di-mount ke container
│   ├── WPS/
│   │   ├── namelist.wps
│   │   ├── geo_em.d0*.nc
│   │   ├── FILE:2017-11-*
...
DTCenter. WPS/WRF Docker Container Documentation — github.com/NCAR/container-wrf-wps
---
